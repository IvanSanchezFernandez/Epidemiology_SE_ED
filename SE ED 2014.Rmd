...
NEDS2014CORESPECIFICATIONS <- read_fwf("D:\\NEDS\\NEDS2014\\FileSpecifications_NEDS_2014_Core.txt",
                                   fwf_positions(
...
colnames(NEDS2014CORESPECIFICATIONS) <- c("Database name", "Discharge year of data", 
...
NEDS2014COREDATABASE <- read_csv("D:\\NEDS\\NEDS2014\\NEDS_2014_CORE.csv",
                             col_names=NEDS2014CORESPECIFICATIONS$`Data element name`)
```
...
# Read in the specifications file
NEDS2014EDSPECIFICATIONS <- read_fwf("D:\\NEDS\\NEDS2014\\FileSpecifications_NEDS_2014_ED.txt",
                                   fwf_positions(
                                     start = c(1, 6, 11, 21, 25, 45, 48, 50, 55),
                                     end =   c(4, 9, 19, 23, 43, 45, 48, 53, 154)
                                   ),
...
# Define the variables names
colnames(NEDS2014EDSPECIFICATIONS) <- c("Database name", "Discharge year of data", 
                                        "File name", "Data element number", 
                                        "Data element name", "Length of data element",
                                        "Non-zero number of digits after decimal point for                                              numeric data element",
                                        "Data element type (Num=numeric; Char=character)",
...
# Read in the database (with data.table fread because it is too large for read_csv)
NEDS2014EDDATABASE <- fread("D:\\NEDS\\NEDS2014\\NEDS_2014_ED.csv",
                             col.names=NEDS2014EDSPECIFICATIONS$`Data element name`)
```




NEDS 2014 HOSPITAL IP
```{r}
## NEDS 2014 IP

# Read in the specifications file
NEDS2014IPSPECIFICATIONS <- read_fwf("D:\\NEDS\\NEDS2014\\FileSpecifications_NEDS_2014_IP.txt",
                                   fwf_positions(
                                     start = c(1, 6, 11, 21, 25, 45, 48, 50, 55),
                                     end =   c(4, 9, 19, 23, 43, 45, 48, 53, 154)
                                   ),
...
# Define the variables names
colnames(NEDS2014IPSPECIFICATIONS) <- c("Database name", "Discharge year of data", 
                                        "File name", "Data element number", 
                                        "Data element name", "Length of data element",
                                        "Non-zero number of digits after decimal point for                                              numeric data element",
                                        "Data element type (Num=numeric; Char=character)",
...
# Read in the database
NEDS2014IPDATABASE <- read_csv("D:\\NEDS\\NEDS2014\\NEDS_2014_IP.csv",
                             col_names=NEDS2014IPSPECIFICATIONS$`Data element name`)
```




NEDS 2014 HOSPITAL FILE
```{r}
## NEDS 2014 HOSPITAL

# Read in the specifications file
NEDS2014HOSPITALSPECIFICATIONS <- read_fwf("D:\\NEDS\\NEDS2014\\FileSpecifications_NEDS_2014_Hospital.txt",
                                   fwf_positions(
                                     start = c(1, 6, 11, 21, 25, 45, 48, 50, 55),
                                     end =   c(4, 9, 19, 23, 43, 45, 48, 53, 154)
                                   ),
...
# Define the variables names
colnames(NEDS2014HOSPITALSPECIFICATIONS) <- c("Database name", "Discharge year of data", 
                                              "File name", "Data element number", 
                                              "Data element name", "Length of data element",
                                              "Non-zero number of digits after decimal point                                                  for numeric data element",
                                              "Data element type (Num=numeric;                                                                Char=character)",
...
# Read in the database
NEDS2014HOSPITALDATABASE <- read_csv("D:\\NEDS\\NEDS2014\\NEDS_2014_HOSPITAL.csv",
                             col_names=NEDS2014HOSPITALSPECIFICATIONS$`Data element name`)
```
...
Keep only patients with status epilepticus and merge into a single database: NEDS 2014 SE
```{r}
## KEEP ONLY PATIENTS WITH STATUS EPILEPTICUS AS ONE OF THE CODES OR SEIZURE AS THE PRIMARY CODE
# Transform linking variable to the same class to avoid misrecognition
NEDS2014EDDATABASE$KEY_ED <- as.numeric(NEDS2014EDDATABASE$KEY_ED)
NEDS2014_SE <- NEDS2014COREDATABASE %>% # SE
  ...
  left_join(., NEDS2014EDDATABASE) %>% # Add the variables for inpatient from the IP database
  left_join(., NEDS2014IPDATABASE) %>% # Add the variables from the HOSPITAL database
  left_join(., NEDS2014HOSPITALDATABASE) 
```
...
rm("NEDS2014COREDATABASE", "NEDS2014CORESPECIFICATIONS", "NEDS2014EDDATABASE",  "NEDS2014EDSPECIFICATIONS", "NEDS2014HOSPITALDATABASE", "NEDS2014HOSPITALSPECIFICATIONS", "NEDS2014IPDATABASE", "NEDS2014IPSPECIFICATIONS")


# Recode type of seizure emergencies
NEDS2014_SE <- NEDS2014_SE %>%
...
NEDS2014_SE$epilepsy <- factor(ifelse(
    grepl("^345$", NEDS2014_SE$DX1) | grepl("^345$", NEDS2014_SE$DX2) |
    grepl("^345$", NEDS2014_SE$DX3) | grepl("^345$", NEDS2014_SE$DX4) |
    grepl("^345$", NEDS2014_SE$DX5) | grepl("^345$", NEDS2014_SE$DX6) |
    grepl("^345$", NEDS2014_SE$DX7) | grepl("^345$", NEDS2014_SE$DX8) |
    grepl("^345$", NEDS2014_SE$DX9) | grepl("^345$", NEDS2014_SE$DX10) |
    grepl("^345$", NEDS2014_SE$DX11) | grepl("^345$", NEDS2014_SE$DX12) |
    grepl("^345$", NEDS2014_SE$DX13) | grepl("^345$", NEDS2014_SE$DX14) |
    grepl("^345$", NEDS2014_SE$DX15) | # 345, ICD9 code for epilepsy and recurrent seizures
    grepl("^3451.*", NEDS2014_SE$DX1) | grepl("^3451.*", NEDS2014_SE$DX2) |
    grepl("^3451.*", NEDS2014_SE$DX3) | grepl("^3451.*", NEDS2014_SE$DX4) |
    grepl("^3451.*", NEDS2014_SE$DX5) | grepl("^3451.*", NEDS2014_SE$DX6) |
    grepl("^3451.*", NEDS2014_SE$DX7) | grepl("^3451.*", NEDS2014_SE$DX8) |
    grepl("^3451.*", NEDS2014_SE$DX9) | grepl("^3451.*", NEDS2014_SE$DX10) |
    grepl("^3451.*", NEDS2014_SE$DX11) | grepl("^3451.*", NEDS2014_SE$DX12) |
    grepl("^3451.*", NEDS2014_SE$DX13) | grepl("^3451.*", NEDS2014_SE$DX14) |
    grepl("^3451.*", NEDS2014_SE$DX15) | # 3451, ICD9 code for generalized convulsive epilepsy
    grepl("^3454.*", NEDS2014_SE$DX1) | grepl("^3454.*", NEDS2014_SE$DX2) |
    grepl("^3454.*", NEDS2014_SE$DX3) | grepl("^3454.*", NEDS2014_SE$DX4) |
    grepl("^3454.*", NEDS2014_SE$DX5) | grepl("^3454.*", NEDS2014_SE$DX6) |
    grepl("^3454.*", NEDS2014_SE$DX7) | grepl("^3454.*", NEDS2014_SE$DX8) |
    grepl("^3454.*", NEDS2014_SE$DX9) | grepl("^3454.*", NEDS2014_SE$DX10) |
    grepl("^3454.*", NEDS2014_SE$DX11) | grepl("^3454.*", NEDS2014_SE$DX12) |
    grepl("^3454.*", NEDS2014_SE$DX13) | grepl("^3454.*", NEDS2014_SE$DX14) |
    grepl("^3454.*", NEDS2014_SE$DX15) | # 3454, ICD9 code for location-related epilepsy
    grepl("^3455.*", NEDS2014_SE$DX1) | grepl("^3455.*", NEDS2014_SE$DX2) |
    grepl("^3455.*", NEDS2014_SE$DX3) | grepl("^3455.*", NEDS2014_SE$DX4) |
    grepl("^3455.*", NEDS2014_SE$DX5) | grepl("^3455.*", NEDS2014_SE$DX6) |
    grepl("^3455.*", NEDS2014_SE$DX7) | grepl("^3455.*", NEDS2014_SE$DX8) |
    grepl("^3455.*", NEDS2014_SE$DX9) | grepl("^3455.*", NEDS2014_SE$DX10) |
    grepl("^3455.*", NEDS2014_SE$DX11) | grepl("^3455.*", NEDS2014_SE$DX12) |
    grepl("^3455.*", NEDS2014_SE$DX13) | grepl("^3455.*", NEDS2014_SE$DX14) |
    grepl("^3455.*", NEDS2014_SE$DX15) | # 3455, ICD9 code for location-related epilepsy      
    grepl("^3456.*", NEDS2014_SE$DX1) | grepl("^3456.*", NEDS2014_SE$DX2) |
    grepl("^3456.*", NEDS2014_SE$DX3) | grepl("^3456.*", NEDS2014_SE$DX4) |
    grepl("^3456.*", NEDS2014_SE$DX5) | grepl("^3456.*", NEDS2014_SE$DX6) |
    grepl("^3456.*", NEDS2014_SE$DX7) | grepl("^3456.*", NEDS2014_SE$DX8) |
    grepl("^3456.*", NEDS2014_SE$DX9) | grepl("^3456.*", NEDS2014_SE$DX10) |
    grepl("^3456.*", NEDS2014_SE$DX11) | grepl("^3456.*", NEDS2014_SE$DX12) |
    grepl("^3456.*", NEDS2014_SE$DX13) | grepl("^3456.*", NEDS2014_SE$DX14) |
    grepl("^3456.*", NEDS2014_SE$DX15) | # 3456, ICD9 code for infantile spasms  
    grepl("^3458.*", NEDS2014_SE$DX1) | grepl("^3458.*", NEDS2014_SE$DX2) |
    grepl("^3458.*", NEDS2014_SE$DX3) | grepl("^3458.*", NEDS2014_SE$DX4) |
    grepl("^3458.*", NEDS2014_SE$DX5) | grepl("^3458.*", NEDS2014_SE$DX6) |
    grepl("^3458.*", NEDS2014_SE$DX7) | grepl("^3458.*", NEDS2014_SE$DX8) |
    grepl("^3458.*", NEDS2014_SE$DX9) | grepl("^3458.*", NEDS2014_SE$DX10) |
    grepl("^3458.*", NEDS2014_SE$DX11) | grepl("^3458.*", NEDS2014_SE$DX12) |
    grepl("^3458.*", NEDS2014_SE$DX13) | grepl("^3458.*", NEDS2014_SE$DX14) |
    grepl("^3458.*", NEDS2014_SE$DX15) | # 3458, ICD9 code for epilepsy and recurrent seizures
    grepl("^3459.*", NEDS2014_SE$DX1) | grepl("^3459.*", NEDS2014_SE$DX2) |
    grepl("^3459.*", NEDS2014_SE$DX3) | grepl("^3459.*", NEDS2014_SE$DX4) |
    grepl("^3459.*", NEDS2014_SE$DX5) | grepl("^3459.*", NEDS2014_SE$DX6) |
    grepl("^3459.*", NEDS2014_SE$DX7) | grepl("^3459.*", NEDS2014_SE$DX8) |
    grepl("^3459.*", NEDS2014_SE$DX9) | grepl("^3459.*", NEDS2014_SE$DX10) |
    grepl("^3459.*", NEDS2014_SE$DX11) | grepl("^3459.*", NEDS2014_SE$DX12) |
    grepl("^3459.*", NEDS2014_SE$DX13) | grepl("^3459.*", NEDS2014_SE$DX14) |
    grepl("^3459.*", NEDS2014_SE$DX15), # 3459, ICD9 code for epilepsy unspecified  
    "yes", "no"))
  

# Recode hospital features
NEDS2014_SE <- NEDS2014_SE %>% # Hospital region
...
NEDS2014_SE <- NEDS2014_SE %>% # Recode NA for AGE missing data (-66)
  ...
  mutate(urban_rural = factor(
    case_when(
      PL_NCHS==1 ~ "Central counties large metro area",
      PL_NCHS==2 ~ "Fringe counties large metro area",
      PL_NCHS==3 ~ "Counties in medium metro area",
      PL_NCHS==4 ~ "Counties in small metro area",
      PL_NCHS==5 ~ "Micropolitan counties",
      PL_NCHS==6 ~ "Not metropolitan or micropolitan counties")
  ))


# Recode covariates
NEDS2014_SE <- NEDS2014_SE %>% # Recode injury
...
NEDS2014_SE <- NEDS2014_SE %>% # Respiratory intubation or mechanical ventilation in the emergency department
...
NEDS2014_SE <- NEDS2014_SE %>% # Routine electroencephalogram in the emergency department
...
NEDS2014_SE <- NEDS2014_SE %>% # Head CT in the emergency room
...
NEDS2014_SE <- NEDS2014_SE %>% # Lumbar puncture in the emergency department
...
# Recode outcomes
NEDS2014_SE <- NEDS2014_SE %>% # Disposition from the ED
...
```{r}
NEDS2014_SE_survey <- NEDS2014_SE %>% as_survey_design(ids=KEY_ED, strata=NEDS_STRATUM, weights=DISCWT, nest=TRUE)
```
...
# Weighted population
total_population_results <- NEDS2014_SE_survey %>% 
...
age_results <- bind_rows(
  NEDS2014_SE_survey %>% summarize(percentile=survey_quantile(AGE, quantiles=c(0.25, 0.5, 0.75), na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type) %>% summarize(percentile=survey_quantile(AGE, quantiles=c(0.25, 0.5, 0.75), na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
age_results

# Age categories
age_categories_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(age_categories) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, age_categories) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
...
sex_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(sex_categories) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, sex_categories) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
sex_results
...
SES_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(SES) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, SES) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
SES_results
...
payer_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(primary_payer) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, primary_payer) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
payer_results
...
weekend_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(admission) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, admission) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
weekend_results
...
hosp_region_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(HOSP_REGION) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, HOSP_REGION) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
hosp_region_results
...
hosp_teaching_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(HOSP_UR_TEACH) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, HOSP_UR_TEACH) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
...
intubation_ED_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(intubation_ED) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, intubation_ED) %>% summarize(
   number=survey_total(na.rm=TRUE, vartype=NULL), 
   proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
...
intubation_IP_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(intubation_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, intubation_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
...
ventilationless96h_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(ventilationless96h) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, ventilationless96h) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
...
ventilationmore96h_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(ventilationmore96h) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, ventilationmore96h) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
...
EEG_IP_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(EEG_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, EEG_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
EEG_IP_results
...
EEGmonitoring_IP_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(EEGmonitoring_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, EEGmonitoring_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
...
headCT_ED_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(headCT_ED) %>% summarize(
  number=survey_total(na.rm=TRUE, vartype=NULL), 
  proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, headCT_ED) %>% summarize(
  number=survey_total(na.rm=TRUE, vartype=NULL), 
  proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
headCT_ED_results
...
headCT_IP_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(headCT_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, headCT_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
headCT_IP_results
...
brainMRI_IP_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(brainMRI_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, brainMRI_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
brainMRI_IP_results
...
LP_ED_results <- bind_rows(
 NEDS2014_SE_survey %>% group_by(LP_ED) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
    NEDS2014_SE_survey %>% group_by(seizure_type, LP_ED) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
LP_ED_results
...
LP_IP_results <- bind_rows(
  NEDS2014_SE_survey %>% group_by(LP_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, LP_IP) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
LP_IP_results
```
...
```{r}
# Principal diagnosis in patients with secondary SE
principal_diagnosis <- NEDS2014_SE_survey %>% 
...
principal_diagnosis1yo <- NEDS2014_SE_survey %>% 
...
principal_diagnosis1_5yo <- NEDS2014_SE_survey %>% 
...
principal_diagnosis6_12yo <- NEDS2014_SE_survey %>% 
...
principal_diagnosis13_20yo <- NEDS2014_SE_survey %>% 
...
principal_diagnosis21_35yo <- NEDS2014_SE_survey %>% 
...
principal_diagnosis36_50yo <- NEDS2014_SE_survey %>% 
...
principal_diagnosis51_65yo <- NEDS2014_SE_survey %>% 
...
principal_diagnosis66_89yo <- NEDS2014_SE_survey %>% 
...
principal_diagnosis89yo <- NEDS2014_SE_survey %>% 
...
ED_disposition <- bind_rows(
  NEDS2014_SE_survey %>% group_by(ED_outcome) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, ED_outcome) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
ED_disposition

# Hospital admission multivariable
hospital_admission_multivariable <- svyglm(ED_hospital_admission ~ seizure_type + age_categories + sex_categories + INJURY + MULTINJURY + NECODE + NDX + primary_payer + urban_rural + SES + HOSP_REGION + HOSP_CONTROL + HOSP_TRAUMA + HOSP_UR_TEACH + hospital_urban_rural, family=quasibinomial, design=NEDS2014_SE_survey)
...
hospital_transfer_multivariable <- svyglm(ED_hospital_transfer ~ seizure_type + age_categories + sex_categories + INJURY + MULTINJURY + NECODE + NDX + primary_payer + urban_rural + SES + HOSP_REGION + HOSP_CONTROL + HOSP_TRAUMA + HOSP_UR_TEACH + hospital_urban_rural, family=quasibinomial, design=NEDS2014_SE_survey)
...
LOS_outcome <- bind_rows(
  NEDS2014_SE_survey %>% summarize(
    survey_quantile(LOS_IP, c(0.25, 0.5, 0.75), vartype=NULL, na.rm=TRUE)), # Total population
  NEDS2014_SE_survey %>% group_by (seizure_type) %>% 
    summarize(
    survey_quantile(LOS_IP, c(0.25, 0.5, 0.75), vartype=NULL, na.rm=TRUE)) # Subgroups by seizure_type
)
LOS_outcome

LOS_multivariable <- svyglm(LOS_IP ~ seizure_type + age_categories + sex_categories + INJURY + MULTINJURY + NECODE + NDX + primary_payer + urban_rural + SES + HOSP_REGION + HOSP_CONTROL + HOSP_TRAUMA + HOSP_UR_TEACH + hospital_urban_rural + intubation_IP + ventilationless96h + ventilationmore96h, design=NEDS2014_SE_survey)
...
death_outcome <- bind_rows(
  NEDS2014_SE_survey %>% group_by(death) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL)), # Total population
  NEDS2014_SE_survey %>% group_by(seizure_type, death) %>% summarize(
    number=survey_total(na.rm=TRUE, vartype=NULL), 
    proportion=survey_mean(na.rm=TRUE, vartype=NULL))) # Subgroups by seizure_type
death_outcome
...
death_multivariable <- svyglm(deathyesno ~ seizure_type + age_categories + sex_categories + INJURY + MULTINJURY + NECODE + NDX + primary_payer + urban_rural + SES + HOSP_REGION + HOSP_CONTROL + HOSP_TRAUMA + HOSP_UR_TEACH + hospital_urban_rural+ intubation_IP + ventilationless96h + ventilationmore96h, family=quasibinomial, design=NEDS2014_SE_survey)
...
charges_multivariable <- svyglm(charges ~ seizure_type + age_categories + sex_categories + INJURY + MULTINJURY + NECODE + NDX + primary_payer + urban_rural + SES + HOSP_REGION + HOSP_CONTROL + HOSP_TRAUMA + HOSP_UR_TEACH + hospital_urban_rural + intubation_IP + ventilationless96h + ventilationmore96h, design=NEDS2014_SE_survey)
...
total_charges_by_subgroup_proportion_outcome
